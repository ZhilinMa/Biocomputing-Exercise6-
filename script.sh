...
var1=$(cat wages.csv | grep -E '[a-z]+,[0-9]+,12' | sort -t ',' -k 4,4n | head -n 2 | tail -n 1 | cut -d ',' -f4) #you want to get the lowest wage here, use (head -n 1) instesd of (tail -n 1) here 
var2=$(cat wages.csv | grep -E '[a-z]+,[0-9]+,16' | sort -t ',' -k 4,4n | head -n 2 | tail -n 1 | cut -d ',' -f4) #same as above
